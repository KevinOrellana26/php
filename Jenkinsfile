pipeline {
    ...
    parameters {
        choice(
            ...
            description: "PHP Pipeline action to apply",
            name: 'action'
        )
        ...
        string(name: 'NODE_NAMESPACE', defaultValue: params.NODE_NAMESPACE ?: 'aplicaciones-comunes', description: 'Namespace for PHP')

        string(name: 'EKS_CLUSTER_NAME', defaultValue: params.EKS_CLUSTER_NAME ?: 'syndeno', description: 'Cluster Name')
    }
        ...
        stage('Build Image'){
            ...
            steps {
                sh(returnStdout: false, returnStatus:true, script: """#!/bin/bash
                    docker build -t php -f images/Dockerfile .    
                """.stripIndent())
            }
        }
